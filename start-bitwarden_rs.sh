...
docker_name="bitwardenrs/server:$VERSION"
#docker_name="bitwardenrs/server:latest"
...
# the bitwarden vhost host should be loaded last
cat <<-EOF >"/etc/apache2/sites-available/zzz-bitwarden_rs.conf"
...
# clean out old filename
if [ -e /etc/apache2/sites-available/bitwarden_rs.conf ]; then
	a2dissite bitwarden_rs
	rm /etc/apache2/sites-available/bitwarden_rs.conf
fi

a2ensite zzz-bitwarden_rs || true
